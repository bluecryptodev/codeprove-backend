...
router.route('/email_check').post((req, res) => {
    ...
    User.findOne({email: email}, (err, response) => {
        if(response !== null) {
            ...
            res.json({status: false, password: password});
        }
    })
})
...
router.route('/signup').post((req, res) => {
    ...
    if(tokenDelta){
        const newUser = new User({
            username, email, password, phone_number, course_list, lecture_list, lecture_content_list, image, doubts_list, bookmark_list, user_level
        })
        newUser.save()
        .then((response) => {
            res.json({success: true, message: response});
...
router.route('/social-signup').post((req, res) => {
    ...
    User.findOne({email: email}, (err, response) => {
        if(response !== null) {
            ...
            const newUser = new User({
                username, email, password, phone_number, course_list, lecture_list, lecture_content_list, image, doubts_list, bookmark_list, user_level, google_id
            })
            newUser.save()
            .then((response) => {
                res.json({success: true, message: response});
    ...
    User.findById(req.params.id, (err, response) => {
        ...
        if(req.body.flg === 'submit_list'){
            ...
            for(var i = 0; i < response.lecture_content_list.length; i++){
                if(req.body.item_id === response.lecture_content_list[i].id){
                    ...
                    if(req.body.code_mode === 'python'){
                        lecture_content_list[i].python_code = req.body.me_code.your
                    }
            ...
            if(!contain_flg) {
                ...
                if(req.body.code_mode === 'python'){
                    lecture_item_add.python_code = req.body.me_code.your
                }
                if(req.body.code_mode === 'c_cpp'){
        ...
        if(req.body.flg === 'puzzle'){
            ...
            for(var i = 0; i < response.lecture_content_list.length; i++){
                if(req.body.item_id === response.lecture_content_list[i].id){
                    ...
                    lecture_content_list[i].submit_status = req.body.submite_result_status;
                    console.log(lecture_content_list[i]);
                    contain_flg = true;
                }
            }
            if(!contain_flg) {
